...
import numpy as np
from util import sample_and_knn_group, knn_group
...
class OwnPCT(nn.Module):
    def __init__(self, samples=[256, 128, 64]):
        ...
        self.sg_knn = SG_knn(s=2048, in_channels=256, out_channels=128)
    ...
    def forward(self, x):
        ...
        xyz1, features0 = self.sg_knn(features0, xyz, k=32) 
        ...
        x = torch.cat([features0, x01,x02, x03, x04], dim=1)
        x = self.linearL(x)
        x_max = torch.max(x, dim=-1)[0]
